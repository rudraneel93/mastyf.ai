...
Closed-loop agentic workflow for Mastyf AI: scan vulnerabilities, evolve policy and LLM layers from real outcomes, and gate releases on corpus + harness evidence.
...
| **Corpus** | CI | 228-entry eval (`MASTYF_AI_DISABLE_SEMANTIC=true pnpm eval`) |
...
| **Auto Threat Research** | Optional (Pro) | Direct `adv-*.json` writes via `auto-threat-research.mjs` when `SWARM_THREAT_RESEARCH_AUTO=true` + `MASTYF_AI_THREAT_RESEARCH_AUTO=true` |
...
- **SemanticAuditor** — `enqueueSemanticAudit` when `MASTYF_AI_SEMANTIC_ASYNC=true`
