...
describe('integration: HttpProxyServer HTTP API', () => {
  const prevDpop = process.env.MASTYF_AI_LEGACY_NO_DPOP;
  ...
  afterAll(async () => {
    ...
    if (prevDpop === undefined) delete process.env.MASTYF_AI_LEGACY_NO_DPOP;
    else process.env.MASTYF_AI_LEGACY_NO_DPOP = prevDpop;
  });
  ...
  describe('URL and headers', () => {
    ...
    it('forwards User-Agent', async () => {
      await request(proxyPort, {
        method: 'GET',
        headers: { 'User-Agent': 'mastyf-ai-test/1.0' },
      });
      expect(echoState.headers['user-agent']).toBe('mastyf-ai-test/1.0');
    });
  });
  ...
  describe('tenant header', () => {
    ...
    beforeEach(async () => {
      process.env.MASTYF_AI_TENANT_ID = 'default';
      proxy = await startProxy(new PolicyEngine(blockPolicy));
      proxyPort = proxy.getPort();
    });
    ...
    it('accepts X-Mastyf-Ai-Tenant header', async () => {
      const res = await request(proxyPort, {
        method: 'POST',
    ...
    it('rejects invalid tenant id', async () => {
      const res = await request(proxyPort, {
        method: 'GET',
        headers: { 'X-Mastyf-Ai-Tenant': 'bad tenant!' },
      });
  ...
  describe('policy enforcement on tools/call', () => {
    ...
    it('blocks denied tool with 403', async () => {
      ...
      expect(res.text).toMatch(/Blocked by Mastyf AI/);
    });
  ...
  describe('HTTP security hardening', () => {
    let proxy: HttpProxyServer;
    let proxyPort = 0;
    const prevMaxBody = process.env.MASTYF_AI_HTTP_MAX_BODY_BYTES;

    beforeEach(async () => {
      process.env.MASTYF_AI_HTTP_MAX_BODY_BYTES = '2048';
      proxy = await startProxy(new PolicyEngine(blockPolicy));
      proxyPort = proxy.getPort();
    });
    ...
    afterEach(async () => {
      await proxy.stop();
      if (prevMaxBody === undefined) delete process.env.MASTYF_AI_HTTP_MAX_BODY_BYTES;
      else process.env.MASTYF_AI_HTTP_MAX_BODY_BYTES = prevMaxBody;
    });
