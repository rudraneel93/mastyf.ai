...
describe('integration: streamable HTTP upstream relay', () => {
  const prevRelay = process.env.MASTYF_AI_STREAMABLE_HTTP_UPSTREAM_RELAY;
  ...
  afterEach(() => {
    if (prevRelay === undefined) delete process.env.MASTYF_AI_STREAMABLE_HTTP_UPSTREAM_RELAY;
    else process.env.MASTYF_AI_STREAMABLE_HTTP_UPSTREAM_RELAY = prevRelay;
    if (prevDlp === undefined) delete process.env.MASTYF_AI_RESPONSE_DLP_MODE;
    else process.env.MASTYF_AI_RESPONSE_DLP_MODE = prevDlp;
  });
