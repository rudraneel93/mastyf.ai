...
describe('mtls-config helm mounts', () => {
  it('exposes default Helm mount paths', () => {
    expect(MTLS_HELM_MOUNT_PATHS.ca).toBe('/etc/mastyf-ai/tls/ca.pem');
    expect(MTLS_HELM_MOUNT_PATHS.cert).toBe('/etc/mastyf-ai/tls/tls.crt');
    expect(MTLS_HELM_MOUNT_PATHS.key).toBe('/etc/mastyf-ai/tls/tls.key');
  });
