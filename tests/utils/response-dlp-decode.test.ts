...
describe('evaluateResponseDlp redaction', () => {
  it('redacts labeled password value preserving label', () => {
    const prev = process.env.MASTYF_AI_RESPONSE_DLP_MODE;
    ...
    try {
      ...
      if (prev === undefined) delete process.env.MASTYF_AI_RESPONSE_DLP_MODE;
      else process.env.MASTYF_AI_RESPONSE_DLP_MODE = prev;
    }
  });
