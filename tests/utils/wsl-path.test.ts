...
describe('wsl-path', () => {
  ...
  it('path-guard blocks sensitive paths after WSL normalization', () => {
    const prev = process.env.MASTYF_AI_WSL_PATH_MAP;
    process.env.MASTYF_AI_WSL_PATH_MAP = 'true';
    const result = evaluatePathGuard(['/mnt/c/Users/x/.ssh/id_rsa']);
    if (prev === undefined) delete process.env.MASTYF_AI_WSL_PATH_MAP;
    else process.env.MASTYF_AI_WSL_PATH_MAP = prev;
