...
describe('federated-data-source', () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.MASTYF_AI_DASHBOARD_DATA_SOURCE;
    delete process.env.MASTYF_AI_AUDIT_SYNC_ENABLED;
  ...
  it('selects local when pref=local', () => {
    process.env.DATABASE_URL = 'postgresql://localhost/test';
    process.env.MASTYF_AI_DASHBOARD_DATA_SOURCE = 'local';
    expect(resolveFederatedMode(null)).toBe('local');
  });

  it('selects sqlite-fleet when multiple fleet paths', () => {
    process.env.MASTYF_AI_FLEET_DB_PATHS = '/a/history.db,/b/history.db';
