...
describe('ai-enabled', () => {
  ...
  it('learning enabled by default', () => {
    ...
    delete process.env.MASTYF_AI_EXPERIMENTAL_AI;
    expect(isAiLearningEnabled()).toBe(true);
  });

  it('learning disabled when MASTYF_AI_AI_ENABLED=false', () => {
    process.env.MASTYF_AI_AI_ENABLED = 'false';
    expect(isAiLearningEnabled()).toBe(false);
  });

  it('auto-apply off by default', () => {
    delete process.env.MASTYF_AI_AI_AUTO_APPLY;
    delete process.env.MASTYF_AI_EXPERIMENTAL_AI;
