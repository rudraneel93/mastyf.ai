...
describe('audit hash chain', () => {
  ...
  afterEach(() => {
    ...
    delete process.env.MASTYF_AI_AUDIT_HASH_CHAIN;
    delete process.env.MASTYF_AI_AUDIT_HASH_CHAIN_SIEM;
    delete process.env.MASTYF_AI_AUDIT_HASH_CHAIN_SIEM_LOG;
  ...
  it('appends SIEM events to chained log when enabled', () => {
    process.env.MASTYF_AI_AUDIT_HASH_CHAIN = 'true';
    const path = join(dir, 'siem.jsonl');
    process.env.MASTYF_AI_AUDIT_HASH_CHAIN_SIEM_LOG = path;
