...
describe('semantic-timeout', () => {
  const prev = process.env.MASTYF_AI_SEMANTIC_TIMEOUT_MS;
  ...
  afterEach(() => {
    vi.useRealTimers();
    if (prev === undefined) delete process.env.MASTYF_AI_SEMANTIC_TIMEOUT_MS;
    else process.env.MASTYF_AI_SEMANTIC_TIMEOUT_MS = prev;
  });

  it('defaults to 500ms', () => {
    delete process.env.MASTYF_AI_SEMANTIC_TIMEOUT_MS;
    expect(getSemanticTimeoutMs()).toBe(500);
  });

  it('instant LLM timeout defaults to 500ms', () => {
    delete process.env.MASTYF_AI_AI_INSTANT_LLM_TIMEOUT_MS;
    expect(getInstantLlmTimeoutMs()).toBe(500);
  });

  it('respects MASTYF_AI_AI_INSTANT_LLM_TIMEOUT_MS', () => {
    process.env.MASTYF_AI_AI_INSTANT_LLM_TIMEOUT_MS = '250';
    expect(getInstantLlmTimeoutMs()).toBe(250);
  });

  it('respects MASTYF_AI_SEMANTIC_TIMEOUT_MS', () => {
    process.env.MASTYF_AI_SEMANTIC_TIMEOUT_MS = '1200';
    expect(getSemanticTimeoutMs()).toBe(1200);
  });

  it('returns fallback on slow operation', async () => {
    process.env.MASTYF_AI_SEMANTIC_TIMEOUT_MS = '100';
