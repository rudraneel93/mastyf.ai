...
describe('field-encryption', () => {
  const prev = process.env.MASTYF_AI_DB_ENCRYPTION_KEY;

  beforeEach(() => {
    process.env.MASTYF_AI_DB_ENCRYPTION_KEY = 'test-key-for-unit-tests-only';
  });

  afterEach(() => {
    if (prev === undefined) delete process.env.MASTYF_AI_DB_ENCRYPTION_KEY;
    else process.env.MASTYF_AI_DB_ENCRYPTION_KEY = prev;
  });
