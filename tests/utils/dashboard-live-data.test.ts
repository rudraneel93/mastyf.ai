...
describe('dashboard-live-data', () => {
  ...
  it('parses cost budget from env', () => {
    const prev = process.env.MASTYF_AI_COST_BUDGET_USD;
    process.env.MASTYF_AI_COST_BUDGET_USD = '99.5';
    expect(parseCostBudgetUsd()).toBe(99.5);
    if (prev === undefined) {
      delete process.env.MASTYF_AI_COST_BUDGET_USD;
    } else {
      process.env.MASTYF_AI_COST_BUDGET_USD = prev;
    }
