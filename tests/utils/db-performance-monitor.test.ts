...
describe('db-performance-monitor', () => {
  const prev = process.env.MASTYF_AI_DB_SLOW_QUERY_MS;
  ...
  afterEach(() => {
    if (prev === undefined) delete process.env.MASTYF_AI_DB_SLOW_QUERY_MS;
    else process.env.MASTYF_AI_DB_SLOW_QUERY_MS = prev;
    vi.restoreAllMocks();
  });

  it('default threshold is 100ms', () => {
    delete process.env.MASTYF_AI_DB_SLOW_QUERY_MS;
