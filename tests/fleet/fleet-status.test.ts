...
describe('fleet status', () => {
  ...
  const prevDb = process.env.MASTYF_AI_DB_PATH;
  ...
  afterEach(() => {
    ...
    if (prevDb === undefined) delete process.env.MASTYF_AI_DB_PATH;
    else process.env.MASTYF_AI_DB_PATH = prevDb;
  ...
  it('aggregates a single sqlite fleet db path', async () => {
    dir = mkdtempSync(join(tmpdir(), 'mastyf-ai-fleet-'));
