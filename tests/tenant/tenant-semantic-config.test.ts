...
describe('tenant semantic config', () => {
  const prev = process.env.MASTYF_AI_TENANT_SEMANTIC_JSON;
  const prevSync = process.env.MASTYF_AI_SEMANTIC_SYNC_RESPONSE;
  ...
  afterEach(() => {
    resetTenantSemanticConfigForTests();
    if (prev) process.env.MASTYF_AI_TENANT_SEMANTIC_JSON = prev;
    if (prevSync) process.env.MASTYF_AI_SEMANTIC_SYNC_RESPONSE = prevSync;
  });
  ...
  it('tenant override enables sync response for one tenant only', () => {
    process.env.MASTYF_AI_TENANT_SEMANTIC_JSON = JSON.stringify({
