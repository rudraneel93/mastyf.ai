...
describe('multi-tenancy', () => {
  describe('resolve-tenant', () => {
    const prevTenant = process.env.MASTYF_AI_TENANT_ID;

    afterEach(() => {
      if (prevTenant === undefined) delete process.env.MASTYF_AI_TENANT_ID;
      else process.env.MASTYF_AI_TENANT_ID = prevTenant;
    });
    ...
    it('resolves header over env', () => {
      process.env.MASTYF_AI_TENANT_ID = 'env-tenant';
      const ctx = resolveTenantContext({
        headers: { 'x-mastyf-ai-tenant': 'header-tenant' },
      });
      expect(ctx).toEqual({ tenantId: 'header-tenant', source: 'header' });
    });

    it('falls back to env/default', () => {
      delete process.env.MASTYF_AI_TENANT_ID;
      expect(resolveTenantId()).toBe(DEFAULT_TENANT_ID);
      process.env.MASTYF_AI_TENANT_ID = 'pod-tenant';
    ...
    it('resolveTenantFromEnv uses MASTYF_AI_TENANT_ID', () => {
      process.env.MASTYF_AI_TENANT_ID = 'env-only';
    ...
    it('resolveCliTenantId requires tenant when multi-tenant mode without env', () => {
      const prevMulti = process.env.MASTYF_AI_MULTI_TENANT_ENABLED;
      const prevTenant = process.env.MASTYF_AI_TENANT_ID;
      delete process.env.MASTYF_AI_TENANT_ID;
      process.env.MASTYF_AI_MULTI_TENANT_ENABLED = 'true';
      expect(() => resolveCliTenantId()).toThrow(InvalidTenantIdError);
      if (prevMulti === undefined) delete process.env.MASTYF_AI_MULTI_TENANT_ENABLED;
      else process.env.MASTYF_AI_MULTI_TENANT_ENABLED = prevMulti;
      if (prevTenant === undefined) delete process.env.MASTYF_AI_TENANT_ID;
      else process.env.MASTYF_AI_TENANT_ID = prevTenant;
      expect(isMultiTenantModeEnabled()).toBe(process.env.MASTYF_AI_MULTI_TENANT_ENABLED === 'true');
    });
  });
  ...
  describe('attack learning isolation', () => {
    ...
    beforeEach(() => {
      root = mkdtempSync(join(tmpdir(), 'mastyf-ai-mt-'));
      process.env.MASTYF_AI_AI_ENABLED = 'true';
      process.env.MASTYF_AI_AI_INSTANT_LEARNING = 'true';
      process.env.MASTYF_AI_AI_ATTACK_STATE_PATH = join(root, 'default-attack.json');
      resetInstantAttackLearningState();
    ...
    afterEach(() => {
      resetInstantAttackLearningState();
      resetBlockLearningDebounce();
      delete process.env.MASTYF_AI_AI_ENABLED;
      delete process.env.MASTYF_AI_AI_INSTANT_LEARNING;
      delete process.env.MASTYF_AI_AI_ATTACK_STATE_PATH;
  ...
  describe('jwt tenant binding', () => {
    ...
    it('uses JWT tenant as authoritative when authenticated', () => {
      ...
      const resolved = resolveProxyTenantId({
        ...
        headers: { 'x-mastyf-ai-tenant': 'acme-corp' },
      });
    ...
    it('rejects header tenant mismatch with JWT in multi-tenant mode', () => {
      ...
      expect(() =>
        resolveProxyTenantId({
          authenticated: true,
          jwtTenantId: 'acme-corp',
          headers: { 'x-mastyf-ai-tenant': 'other-corp' },
        }),
      ).toThrow(JwtTenantRequiredError);
    });
  });
