...
describe('dashboard-rbac', () => {
  ...
  it('parses MASTYF_AI_DASHBOARD_ROLES comma map', () => {
  ...
  describe('DashboardAuth roles', () => {
    ...
    const prevRoles = process.env.MASTYF_AI_DASHBOARD_ROLES;

    beforeEach(() => {
      process.env.DASHBOARD_API_KEY = 'test-admin-key';
      process.env.MASTYF_AI_DASHBOARD_ROLES = 'test-admin-key:admin,read-only:viewer';
    });

    afterEach(() => {
      if (prevKey === undefined) delete process.env.DASHBOARD_API_KEY;
      else process.env.DASHBOARD_API_KEY = prevKey;
      if (prevRoles === undefined) delete process.env.MASTYF_AI_DASHBOARD_ROLES;
      else process.env.MASTYF_AI_DASHBOARD_ROLES = prevRoles;
    });
