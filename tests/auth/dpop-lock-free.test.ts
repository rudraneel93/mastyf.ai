...
describe('DPoP lock-free jti claim', () => {
  it('enables lock-free mode by default', () => {
    const prev = process.env.MASTYF_AI_DPOP_LOCK_FREE;
    delete process.env.MASTYF_AI_DPOP_LOCK_FREE;
    expect(isDpopLockFreeEnabled()).toBe(true);
    process.env.MASTYF_AI_DPOP_LOCK_FREE = 'legacy';
    expect(isDpopLockFreeEnabled()).toBe(false);
    if (prev !== undefined) process.env.MASTYF_AI_DPOP_LOCK_FREE = prev;
    else delete process.env.MASTYF_AI_DPOP_LOCK_FREE;
  });
