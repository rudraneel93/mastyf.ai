...
describe('dashboard multi-tenant isolation', () => {
  ...
  afterAll(async () => {
    ...
    delete process.env.MASTYF_AI_WS_ENABLED;
    delete process.env.MASTYF_AI_MULTI_TENANT_ENABLED;
  });
  ...
  it('scopes /api/aggregate/audit by X-Mastyf-Ai-Tenant', async () => {
    const resA = await fetch(`http://127.0.0.1:${PORT}/api/aggregate/audit`, {
      headers: { 'X-Mastyf-Ai-Tenant': 'tenant-a' },
    });
    ...
    const resB = await fetch(`http://127.0.0.1:${PORT}/api/aggregate/audit`, {
      headers: { 'X-Mastyf-Ai-Tenant': 'tenant-b' },
    });
  ...
  it('scopes semantic outcomes by tenant', async () => {
    ...
    const semLine = (tenantId: string, toolName: string) => {
      const dir = join(tmpDir, '.mastyf-ai', 'tenants', tenantId);
