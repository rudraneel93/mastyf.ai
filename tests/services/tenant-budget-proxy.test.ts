...
describe('tenant-budget hot path', () => {
  const prev = process.env.GUARDIAN_TENANT_DAILY_BUDGET_JSON;

  beforeEach(() => {
    resetTenantBudgetCacheForTests();
    process.env.GUARDIAN_TENANT_DAILY_BUDGET_JSON = JSON.stringify({ acme: 0.01 });
  });

  afterEach(() => {
    resetTenantBudgetCacheForTests();
    if (prev === undefined) delete process.env.GUARDIAN_TENANT_DAILY_BUDGET_JSON;
    else process.env.GUARDIAN_TENANT_DAILY_BUDGET_JSON = prev;
  });
