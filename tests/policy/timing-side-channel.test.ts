...
describe('timing side-channel (enterprise)', () => {
  beforeEach(() => {
    resetTimingProbeCounters();
    process.env['MASTYF_AI_POLICY_TIMING_ENVELOPE'] = 'true';
    process.env['MASTYF_AI_POLICY_MIN_EVAL_MS'] = '20';
  });

  afterEach(() => {
    delete process.env['MASTYF_AI_POLICY_TIMING_ENVELOPE'];
    delete process.env['MASTYF_AI_POLICY_MIN_EVAL_MS'];
  });
  ...
  describe('policy timing envelope', () => {
    ...
    it('can be disabled via MASTYF_AI_POLICY_TIMING_ENVELOPE=false', () => {
