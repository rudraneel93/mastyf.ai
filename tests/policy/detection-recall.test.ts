...
describe('detection-recall', () => {
  ...
  describe('corpus regression (PolicyEngine, regex-only)', () => {
    it('blocks every attack fixture with zero benign false positives', async () => {
      const prev = process.env.MASTYF_AI_DISABLE_SEMANTIC;
      process.env.MASTYF_AI_DISABLE_SEMANTIC = 'true';
      try {
        ...
        if (prev === undefined) delete process.env.MASTYF_AI_DISABLE_SEMANTIC;
        else process.env.MASTYF_AI_DISABLE_SEMANTIC = prev;
      }
    });
  });
});
