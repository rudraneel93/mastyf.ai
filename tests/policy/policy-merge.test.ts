...
describe('policy-merge', () => {
  const prev = process.env.MASTYF_AI_HTTP_TOOLS_POLICY;

  afterEach(() => {
    if (prev === undefined) delete process.env.MASTYF_AI_HTTP_TOOLS_POLICY;
    else process.env.MASTYF_AI_HTTP_TOOLS_POLICY = prev;
  });

  it('does not merge when MASTYF_AI_HTTP_TOOLS_POLICY is unset', () => {
    delete process.env.MASTYF_AI_HTTP_TOOLS_POLICY;
  ...
  it('merges http-tools template when enabled', () => {
    process.env.MASTYF_AI_HTTP_TOOLS_POLICY = 'true';
