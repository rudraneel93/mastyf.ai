...
describe('OPA / YAML precedence', () => {
  ...
  const originalStrict = process.env['MASTYF_AI_STRICT_MODE'];
  ...
  afterEach(() => {
    ...
    if (originalStrict === undefined) delete process.env['MASTYF_AI_STRICT_MODE'];
    else process.env['MASTYF_AI_STRICT_MODE'] = originalStrict;
  });
