...
describe('OPA response cache', () => {
  ...
  const prevTtl = process.env['MASTYF_AI_OPA_CACHE_TTL_MS'];

  beforeEach(() => {
    resetOpaCacheForTests();
    process.env['OPA_URL'] = 'http://opa.test/v1/data/mcp';
    process.env['MASTYF_AI_OPA_CACHE_TTL_MS'] = '60000';
  });
  ...
  afterEach(() => {
    ...
    if (prevTtl === undefined) delete process.env['MASTYF_AI_OPA_CACHE_TTL_MS'];
    else process.env['MASTYF_AI_OPA_CACHE_TTL_MS'] = prevTtl;
  });
