...
describe('enterprise five mitigations', () => {
  ...
  describe('4. timing-based side channels', () => {
    ...
    it('PolicyEngine applies min-eval timing envelope on pass', () => {
      const prev = process.env.MASTYF_AI_POLICY_TIMING_ENVELOPE;
      ...
      try {
        ...
        if (prev === undefined) delete process.env.MASTYF_AI_POLICY_TIMING_ENVELOPE;
        else process.env.MASTYF_AI_POLICY_TIMING_ENVELOPE = prev;
      }
    });
  });
