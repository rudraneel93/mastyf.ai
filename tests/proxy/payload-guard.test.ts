...
describe('payload guard', () => {
  it('rejects oversized raw payload', () => {
    const prev = process.env.MASTYF_AI_MAX_PAYLOAD_BYTES;
    process.env.MASTYF_AI_MAX_PAYLOAD_BYTES = '100';
    const big = 'x'.repeat(200);
    expect(checkRawPayloadSize(big).ok).toBe(false);
    if (prev === undefined) delete process.env.MASTYF_AI_MAX_PAYLOAD_BYTES;
    else process.env.MASTYF_AI_MAX_PAYLOAD_BYTES = prev;
  });

  it('rejects expanded args over limit', () => {
    const prev = process.env.MASTYF_AI_MAX_EXPANDED_PAYLOAD_BYTES;
    process.env.MASTYF_AI_MAX_EXPANDED_PAYLOAD_BYTES = '50';
    expect(checkExpandedPayload({ data: 'x'.repeat(100) }).ok).toBe(false);
    if (prev === undefined) delete process.env.MASTYF_AI_MAX_EXPANDED_PAYLOAD_BYTES;
    else process.env.MASTYF_AI_MAX_EXPANDED_PAYLOAD_BYTES = prev;
  });
});
