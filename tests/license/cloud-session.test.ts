...
describe('cloud-session', () => {
  ...
  it('verifies tokens signed with GUARDIAN_CLOUD_JWT_SECRET', () => {
    process.env.GUARDIAN_CLOUD_JWT_SECRET = 'guardian-test-secret';
    const payload = {
      tenantSlug: 'acme',
      identity: 'cloud:acme',
      roles: ['tenant-admin'],
    ...
    const sig = createHmac('sha256', 'guardian-test-secret').update(encoded).digest('base64url');
