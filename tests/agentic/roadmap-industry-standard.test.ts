...
describe('Phase 2 roadmap modules', () => {
  ...
  it('C3 zero-trust engine returns composite score with SPIFFE dimension', () => {
    ...
    const score = engine.score({
      ...
      toolName: 'read_file',
      authenticated: true,
      spiffeId: 'spiffe://mastyf-ai/agent/a1',
    });
...
describe('Phase 3 roadmap modules', () => {
  ...
  it('B2 observatory snapshot aggregates metrics', () => {
    ...
    ingestMastyfAiBenchIntoObservatory(obs, { blockRate: 0.9, falsePositiveRate: 0.02, serverCount: 3 });
  ...
  it('C4 insurance quantifier computes ALE and exports PDF', () => {
    ...
    const prev = process.env.MASTYF_AI_INSURANCE_REPORT_DIR;
    process.env.MASTYF_AI_INSURANCE_REPORT_DIR = join(process.cwd(), 'reports', 'insurance-test');
    const pdf = writeInsuranceRiskPdf(report);
    expect(pdf.path).toMatch(/\.pdf$/);
    expect(pdf.pdfBase64.length).toBeGreaterThan(100);
    process.env.MASTYF_AI_INSURANCE_REPORT_DIR = prev;
  });
});
...
describe('A1 SIEM export', () => {
  it('formats fleet chain alert as CEF', () => {
    ...
    expect(line).toMatch(/^CEF:0\|Mastyf AI\|/);
...
describe('SPIFFE identity', () => {
  it('reads SPIFFE ID from env', () => {
    const prev = process.env.MASTYF_AI_SPIFFE_ID;
    process.env.MASTYF_AI_SPIFFE_ID = 'spiffe://example.org/agent/test';
    expect(getActiveSpiffeId()).toBe('spiffe://example.org/agent/test');
    process.env.MASTYF_AI_SPIFFE_ID = prev;
  });
});
