...
describe('plan compliance phase 2 (100%)', () => {
  ...
  it('B3 secure weight aggregation powers federated inference', async () => {
    ...
    process.env.MASTYF_AI_FEDERATED_LEARNING_MIN_REPORTS = '1';
  ...
  it('A3 honors MASTYF_AI_BIOMETRICS_MIN_SAMPLES for warm-up', () => {
    process.env.MASTYF_AI_BIOMETRICS_MIN_SAMPLES = '3';
    ...
    delete process.env.MASTYF_AI_BIOMETRICS_MIN_SAMPLES;
  });
  ...
  it('C3 SPIFFE workload identity elevates zero-trust score', () => {
    process.env.MASTYF_AI_SPIFFE_SOCKET_PATH = '/run/spire/sockets/agent.sock';
    ...
    delete process.env.MASTYF_AI_SPIFFE_SOCKET_PATH;
  });
  it('B1 certifier downgrades when network reputation disagrees', () => {
    ...
    net.rateServer({
      ...
      dimensions: {
        ...
        policy_compliance: 30, uptime: 30, community_rating: 30, mastyf_ai_protected: 30,
      },
    });
  ...
  it('A1 multi-region fleet redis keys include region tag', () => {
    process.env.MASTYF_AI_FLEET_REGION = 'us-east-1';
    expect(fleetRegion()).toBe('US-EAST-1');
    process.env.MASTYF_AI_FLEET_PEER_REGIONS = 'eu-west-1';
    expect(fleetPeerRegions()).toContain('EU-WEST-1');
    delete process.env.MASTYF_AI_FLEET_REGION;
    delete process.env.MASTYF_AI_FLEET_PEER_REGIONS;
  });
  ...
  it('B3 MPC-lite masked gradients cancel pairwise masks', () => {
    process.env.MASTYF_AI_FEDERATED_MPC_SECRET = 'test-secret';
    ...
    delete process.env.MASTYF_AI_FEDERATED_MPC_SECRET;
  });
