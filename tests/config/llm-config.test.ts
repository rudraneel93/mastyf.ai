...
describe('llm-config', () => {
  const keys = [
    'MASTYF_AI_LLM_PROVIDER',
    'MASTYF_AI_LLM_MODEL',
    'ANTHROPIC_API_KEY',
    'OPENAI_API_KEY',
    'OLLAMA_BASE_URL',
    'MASTYF_AI_LLM_MAX_TOKENS',
    'MASTYF_AI_LLM_TIMEOUT_MS',
    'MASTYF_AI_LLM_TEMPERATURE',
    'MASTYF_AI_MODEL',
  ] as const;
  ...
  it('reads max tokens, timeout, and temperature from env', () => {
    process.env.MASTYF_AI_LLM_MAX_TOKENS = '2048';
    process.env.MASTYF_AI_LLM_TIMEOUT_MS = '15000';
    process.env.MASTYF_AI_LLM_TEMPERATURE = '0.5';
    const cfg = getLlmConfig();
  ...
  it('resolveModelId prefers payload then MASTYF_AI_MODEL then config default', () => {
    process.env.MASTYF_AI_LLM_PROVIDER = 'openai';
    process.env.MASTYF_AI_LLM_MODEL = 'gpt-4o-mini';
    expect(resolveModelId('from-payload')).toBe('from-payload');
    delete process.env.MASTYF_AI_MODEL;
    expect(resolveModelId()).toBe('gpt-4o-mini');
    process.env.MASTYF_AI_MODEL = 'gpt-4o';
    expect(resolveModelId()).toBe('gpt-4o');
  });

  it('resolveModelIdForServer prefers server env and MASTYF_AI_MODEL_<SERVER>', () => {
    process.env.MASTYF_AI_LLM_MODEL = 'gpt-4o-mini';
    expect(resolveModelIdForServer('my-server', { MASTYF_AI_MODEL: 'claude-3-5-sonnet' })).toBe(
      'claude-3-5-sonnet',
    );
    delete process.env.MASTYF_AI_MODEL_MY_SERVER;
    process.env.MASTYF_AI_MODEL_MY_SERVER = 'gpt-4o';
