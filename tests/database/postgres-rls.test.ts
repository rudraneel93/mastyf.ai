...
describe('Postgres RLS session', () => {
  it('is disabled unless MASTYF_AI_PG_RLS_ENABLED=true', () => {
    const prev = process.env.MASTYF_AI_PG_RLS_ENABLED;
    delete process.env.MASTYF_AI_PG_RLS_ENABLED;
    expect(isPostgresRlsEnabled()).toBe(false);
    process.env.MASTYF_AI_PG_RLS_ENABLED = 'true';
    expect(isPostgresRlsEnabled()).toBe(true);
    if (prev !== undefined) process.env.MASTYF_AI_PG_RLS_ENABLED = prev;
    else delete process.env.MASTYF_AI_PG_RLS_ENABLED;
  });
