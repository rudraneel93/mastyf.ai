...
import { buildGuardianFullAnalysis } from '../../src/ai/guardian-full-analysis.js';
...
describe('buildGuardianFullAnalysis', () => {
  beforeEach(() => {
    vi.stubEnv('GUARDIAN_FULL_ANALYSIS_LLM', 'false');
  });
  ...
  it('returns measured analysis with sections and markdown', async () => {
    ...
    const result = await buildGuardianFullAnalysis(db, 'default', {
      windowDays: 7,
      useLlm: false,
    ...
    expect(result!.markdown).toContain('# MCP Guardian — Full Analysis');
    expect(result!.plainEnglishSummary.length).toBeGreaterThan(10);
    expect(result!.citations.length).toBeGreaterThan(0);
  });

  it('returns null when db is null', async () => {
    expect(await buildGuardianFullAnalysis(null, 'default')).toBeNull();
  });
});
