...
describe('llm-cache', () => {
  ...
  const prevCacheFlag = process.env.MASTYF_AI_LLM_CACHE;
  const prevTtl = process.env.MASTYF_AI_LLM_CACHE_TTL_SEC;

  beforeEach(() => {
    resetLlmCacheForTests();
    resetLlmConfigForTests();
    delete process.env.REDIS_URL;
    process.env.MASTYF_AI_LLM_CACHE = 'true';
    process.env.MASTYF_AI_LLM_CACHE_TTL_SEC = '60';
  });
  ...
  afterEach(() => {
    ...
    if (prevCacheFlag === undefined) delete process.env.MASTYF_AI_LLM_CACHE;
    else process.env.MASTYF_AI_LLM_CACHE = prevCacheFlag;
    if (prevTtl === undefined) delete process.env.MASTYF_AI_LLM_CACHE_TTL_SEC;
    else process.env.MASTYF_AI_LLM_CACHE_TTL_SEC = prevTtl;
  });
  ...
  it('disables cache when MASTYF_AI_LLM_CACHE=false', () => {
    process.env.REDIS_URL = 'redis://127.0.0.1:6379';
  ...
  it('forces LRU-only path when MASTYF_AI_LLM_CACHE=true without REDIS_URL', async () => {
    process.env.MASTYF_AI_LLM_CACHE = 'true';
    delete process.env.REDIS_URL;
    const cache = getLlmCache();
    const key = {
