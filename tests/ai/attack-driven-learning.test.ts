...
describe('attack-driven learning', () => {
  ...
  it('recordPolicyDecisionGlobal feeds collector metadata', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'mastyf-ai-adl-'));
  ...
  it('runLearningCycle surfaces attack suggestions from blocked call_records', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'mastyf-ai-adl-cycle-'));
    const pendingPath = join(dir, '.ai-pending-suggestions.json');
    process.env.MASTYF_AI_AI_SUGGESTIONS_PATH = pendingPath;
    process.env.MASTYF_AI_AI_STATE_PATH = join(dir, '.ai-learning.json');
    process.env.MASTYF_AI_AI_BASELINES_PATH = join(dir, '.ai-baselines.json');
  ...
  it('recordBlockLearningEvent updates instant state synchronously', () => {
    const dir = mkdtempSync(join(tmpdir(), 'mastyf-ai-adl-instant-'));
  ...
  it('onPolicyBlock schedules debounced flush without throwing', async () => {
    vi.useFakeTimers();
    process.env.MASTYF_AI_AI_BLOCK_DEBOUNCE_MS = '50';
    onPolicyBlock({
      block_rule: 'secret-scan',
      toolName: 'write_file',
      serverName: 'filesystem',
  ...
  it('applySuggestionToPolicy merges rule without duplicates', async () => {
    process.env.MASTYF_AI_POLICY_SIM_GATE = 'false';
    const dir = mkdtempSync(join(tmpdir(), 'mastyf-ai-policy-'));
