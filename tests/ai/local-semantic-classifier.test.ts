...
describe('local-semantic-classifier', () => {
  ...
  it('is enabled by default unless explicitly disabled', () => {
    const prev = process.env.MASTYF_AI_LOCAL_SEMANTIC;
    delete process.env.MASTYF_AI_LOCAL_SEMANTIC;
    expect(isLocalSemanticEnabled()).toBe(true);
    process.env.MASTYF_AI_LOCAL_SEMANTIC = 'false';
    expect(isLocalSemanticEnabled()).toBe(false);
    if (prev === undefined) delete process.env.MASTYF_AI_LOCAL_SEMANTIC;
    else process.env.MASTYF_AI_LOCAL_SEMANTIC = prev;
  });
});
