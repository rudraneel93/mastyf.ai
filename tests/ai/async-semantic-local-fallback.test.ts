...
describe('async-semantic local fallback', () => {
  ...
  const prevLocal = process.env.MASTYF_AI_LOCAL_SEMANTIC;
  ...
  afterEach(() => {
    ...
    if (prevLocal === undefined) delete process.env.MASTYF_AI_LOCAL_SEMANTIC;
    else process.env.MASTYF_AI_LOCAL_SEMANTIC = prevLocal;
