...
describe('instant attack learning', () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), 'mastyf-ai-instant-'));
    process.env.MASTYF_AI_AI_ENABLED = 'true';
    process.env.MASTYF_AI_AI_INSTANT_LEARNING = 'true';
    process.env.MASTYF_AI_AI_ATTACK_MIN_BLOCKS = '3';
    process.env.MASTYF_AI_AI_INSTANT_WINDOW_MS = '300000';
    process.env.MASTYF_AI_AI_ATTACK_STATE_PATH = join(dir, '.attack-learning-state.json');
  ...
  it('updates attack-learning-state.json after each block', () => {
    ...
    expect(existsSync(process.env.MASTYF_AI_AI_ATTACK_STATE_PATH!)).toBe(true);
  ...
  it('queues suggestion immediately after 3 blocks of same rule+tool', () => {
    ...
    const pending = JSON.parse(readFileSync(process.env.MASTYF_AI_AI_SUGGESTIONS_PATH!, 'utf-8'));
  ...
  it('times out slow instant LLM per MASTYF_AI_AI_INSTANT_LLM_TIMEOUT_MS', async () => {
    vi.useFakeTimers();
    process.env.MASTYF_AI_AI_INSTANT_LLM = 'true';
    process.env.MASTYF_AI_AI_INSTANT_LLM_TIMEOUT_MS = '50';
    process.env.MASTYF_AI_AI_INSTANT_LLM_RATE_MS = '0';
