...
describe('threat-research-pipeline', () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), 'mastyf-ai-threat-research-'));
    ...
    process.env.MASTYF_AI_AUTO_CORPUS_MANIFEST = join(dir, 'auto-corpus-manifest.json');
    process.env.MASTYF_AI_THREAT_RESEARCH_STATE_PATH = dir;
    process.env.MASTYF_AI_THREAT_RESEARCH_REQUIRE_REPLAY = 'false';
    process.env.MASTYF_AI_THREAT_RESEARCH_MAX_PER_HOUR = '20';
    vi.mocked(threatLab.discoverFromBypass).mockResolvedValue(validDiscovery as threatLab.ThreatLabDiscovery);
  });

  afterEach(() => {
    resetThreatResearchQueueForTests();
    delete process.env.MASTYF_AI_THREAT_RESEARCH_AUTO;
    ...
    delete process.env.MASTYF_AI_THREAT_RESEARCH_REQUIRE_REPLAY;
    delete process.env.MASTYF_AI_THREAT_RESEARCH_MAX_PER_HOUR;
  ...
  it('enforces hourly rate limit', async () => {
    process.env.MASTYF_AI_THREAT_RESEARCH_MAX_PER_HOUR = '1';
