...
import { buildMastyfAiFullAnalysis } from '../../src/ai/mastyf-ai-full-analysis.js';
...
describe('buildMastyfAiFullAnalysis', () => {
  beforeEach(() => {
    vi.stubEnv('MASTYF_AI_FULL_ANALYSIS_LLM', 'false');
  });
  ...
  it('returns measured analysis with sections and markdown', async () => {
    ...
    const result = await buildMastyfAiFullAnalysis(db, 'default', {
      windowDays: 7,
      useLlm: false,
    ...
    expect(result!.markdown).toContain('# MCP Mastyf AI — Full Analysis');
    expect(result!.plainEnglishSummary.length).toBeGreaterThan(10);
    expect(result!.citations.length).toBeGreaterThan(0);
  });

  it('returns null when db is null', async () => {
    expect(await buildMastyfAiFullAnalysis(null, 'default')).toBeNull();
  });
});
