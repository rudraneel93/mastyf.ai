...
describe('evasion manifest signing', () => {
  const prev = process.env.MASTYF_AI_SWARM_EVASION_SIGNING_KEY;

  beforeEach(() => {
    process.env.MASTYF_AI_SWARM_EVASION_SIGNING_KEY = 'test-signing-key';
  });

  afterEach(() => {
    if (prev === undefined) delete process.env.MASTYF_AI_SWARM_EVASION_SIGNING_KEY;
    else process.env.MASTYF_AI_SWARM_EVASION_SIGNING_KEY = prev;
  });
