...
export function isLocalSemanticEnabledGlobal(): boolean {
  if (process.env['MASTYF_AI_LOCAL_SEMANTIC'] === 'false') return false;
  if (process.env['MASTYF_AI_LOCAL_SEMANTIC'] === 'true') return true;
...
export function isSemanticAsyncEnabledForTenant(tenantId?: string): boolean {
  ...
  if (process.env['MASTYF_AI_SEMANTIC_ASYNC'] === 'false') return false;
  if (process.env['MASTYF_AI_SEMANTIC_ASYNC'] === 'true') return true;
...
export function isSyncSemanticRequestLlmEnabledForTenant(tenantId?: string): boolean {
  ...
  if (process.env['MASTYF_AI_SEMANTIC_SYNC_REQUEST_LLM'] === 'false') return false;
  if (process.env['MASTYF_AI_SEMANTIC_SYNC_REQUEST_LLM'] === 'true') return true;
