...
const MIN_BLOB_LEN = parseInt(process.env.MASTYF_AI_ENTROPY_MIN_LENGTH || '48', 10);
const ENTROPY_THRESHOLD = parseFloat(process.env.MASTYF_AI_ENTROPY_THRESHOLD || '4.2');
...
export function isProxyEntropyCheckEnabled(policyMode?: string): boolean {
  if (process.env.MASTYF_AI_PROXY_ENTROPY === 'false') return false;
  if (process.env.MASTYF_AI_PROXY_ENTROPY === 'true') return true;
