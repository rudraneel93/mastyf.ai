...
function shouldUseRedisTls(url?: string): boolean {
  if (process.env['MASTYF_AI_REDIS_TLS'] === 'true') return true;
  if (process.env['MASTYF_AI_REDIS_TLS'] === 'false') return false;
...
function tlsOptions(): Pick<RedisOptions, 'tls'> | Record<string, never> {
  ...
  return {
    tls: {
      rejectUnauthorized: process.env['MASTYF_AI_REDIS_TLS_REJECT_UNAUTHORIZED'] !== 'false',
    },
  };
}
...
export function createRedisClient(options?: RedisClientOptions): Redis | Cluster {
  ...
  if (process.env['MASTYF_AI_REDIS_TLS'] === 'true' && url.startsWith('redis://')) {
