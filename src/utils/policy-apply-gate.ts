...
export async function requirePolicySimulationBeforeApply(opts: {
  ...
  if (opts.skip || process.env.MASTYF_AI_POLICY_SIM_GATE === 'false') {
    return { allowed: true };
  }
  ...
  try {
    ...
    const blockDelta =
      report.counterfactual.sampleCount > 0
        ...
        : 0;
    const maxFp = parseFloat(process.env.MASTYF_AI_POLICY_SIM_MAX_FP || '0.15');
    const maxBlockDelta = parseFloat(process.env.MASTYF_AI_POLICY_SIM_MAX_BLOCK_DELTA || '0.25');
    ...
    if (process.env.MASTYF_AI_POLICY_SIM_FAIL_OPEN === 'true') {
