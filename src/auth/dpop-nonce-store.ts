...
export function isDpopLockFreeEnabled(): boolean {
  if (process.env['MASTYF_AI_DPOP_LOCK_FREE'] === 'false') return false;
  if (process.env['MASTYF_AI_DPOP_LOCK_FREE'] === 'true') return true;
...
export class RedisDPoPNonceStore implements DPoPNonceStore {
  ...
  private readonly prefix = 'mastyf_ai:dpop:jti:';
...
export function createDPoPNonceStore(ttlMs: number): DPoPNonceStore {
  ...
  if (process.env['MASTYF_AI_CLUSTER_MODE'] === 'true' || process.env['KUBERNETES_SERVICE_HOST']) {
    Logger.warn(
      '[dpop] Using in-memory DPoP nonce store in clustered deployment — set REDIS_URL or MASTYF_AI_DPOP_REDIS_URL for replay protection across instances',
    );
  }
