...
export class MCPCertifier {
  ...
  constructor(
    ...
    this.mastyfAiScore = mastyfAiScore ?? new MastyfAiScore();
  }

  /** Auto-collect certification inputs from MastyfAiScore + CVE posture. */
  collectFromMastyfAiScore(input: Partial<ScoreInput> & { serverName: string }): CertifyManualInputs {
    const score = this.mastyfAiScore.compute({
      ...
      mastyfAiProtected: input.mastyfAiProtected ?? true,
    });
  ...
  certifyFromScan(serverName: string, packageName: string, version: string, scan: Partial<ScoreInput>): CertificationResult {
    const inputs = this.collectFromMastyfAiScore({ ...scan, serverName });
