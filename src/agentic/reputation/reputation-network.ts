...
export class ReputationNetwork {
  ...
  constructor(
    ...
    private readonly mastyfAiScore?: MastyfAiScore,
  ) {}
  ...
  rateServer(params: {
    ...
    const raterId = params.raterId ?? process.env.MASTYF_AI_TENANT_ID ?? 'local-rater';
    ...
    const defaults: ReputationDimensions = {
      ...
      uptime: 50,
      community_rating: 50,
      mastyf_ai_protected: existing ? 0 : 100,
    };
  ...
  ingestRemoteRating(jws: string): { ok: boolean; entry?: ReputationEntry; reason?: string } {
    ...
    if (raterId !== anchor && edges.length > 0) {
      ...
      if (trust < Number(process.env.MASTYF_AI_REPUTATION_MIN_TRUST ?? '0.35')) {
  ...
  async queryWithNetwork(serverName: string, packageName?: string): Promise<ReputationEntry | null> {
    ...
    const relayUrl = process.env.MASTYF_AI_REPUTATION_RELAY_URL?.trim()
  ...
  buildFromMastyfAiScore(serverName: string, score: ReturnType<MastyfAiScore['compute']>): ReputationEntry {
    ...
    return this.rateServer({
      ...
      dimensions: {
        ...
        mastyf_ai_protected: score.includesLiveData ? 100 : 50,
      },
    });
  }
