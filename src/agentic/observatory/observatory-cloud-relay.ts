/**
 * B2 — Pull anonymized ecosystem telemetry from Mastyf AI Cloud / observatory relay.
 */
  ...
  try {
    const res = await fetch(url, {
      ...
      headers: {
        ...
        ...(process.env.MASTYF_AI_OBSERVATORY_RELAY_API_KEY
          ? { Authorization: `Bearer ${process.env.MASTYF_AI_OBSERVATORY_RELAY_API_KEY}` }
          : {}),
      },
      signal: AbortSignal.timeout(Number(process.env.MASTYF_AI_OBSERVATORY_RELAY_TIMEOUT_MS ?? 10_000)),
    });
    if (!res.ok) {
