/**
 * Unified full Guardian analysis — measured facts + optional Ollama plain-English narrative.
 */
...
export type GuardianFullAnalysis = {
  generatedAt: string;
  windowDays: number;
...
function isFullAnalysisLlmEnabled(useLlm: boolean): boolean {
  ...
  if (process.env.GUARDIAN_FULL_ANALYSIS_LLM === 'false') return false;
  return process.env.GUARDIAN_FULL_ANALYSIS_LLM === 'true' || true;
}
...
function buildDeterministicSummary(
  ...
  const detail = bullets[0] || 'No proxy traffic recorded yet — route MCP clients through Guardian.';
  return `${lead} ${detail}`;
}
...
async function buildLlmNarrative(
  citations: FullAnalysisCitation[],
  sections: GuardianFullAnalysis['sections'],
  ...
  try {
    const result = await llm.generate(
      'You explain MCP Guardian security and health to a non-technical operator. Use ONLY the cited facts. Write 6-10 short paragraphs in plain English covering: Protection status, Traffic, Security blocks, Learning/autopilot, and Recommended next steps. Every paragraph must include at least one citation ID in brackets like [exec:requests]. Do not invent numbers or server names.',
      `Facts:\n${corpus}\n\nSection notes:\n${sectionHints}\n\nWrite the full plain-English analysis:`,
    );
    if (!result?.text?.trim()) return null;
    if (!validateLlmCitations(result.text, citations)) {
      Logger.debug('[guardian-full-analysis] LLM narrative rejected — missing citation IDs');
      return null;
    }
    return { narrative: result.text.trim(), provider: 'ollama', model: result.model };
  } catch (err) {
    Logger.debug(
      `[guardian-full-analysis] LLM skipped: ${err instanceof Error ? err.message : String(err)}`,
    );
    return null;
  }
}

function formatMarkdown(report: GuardianFullAnalysis): string {
  const lines: string[] = [
    '# MCP Guardian — Full Analysis',
    '',
  ...
  const sectionTitles: Array<[keyof GuardianFullAnalysis['sections'], string]> = [
  ...
  lines.push('', '---', '_Generated by MCP Guardian — measured aggregates with optional local LLM narrative._');
  return lines.join('\n');
}

export async function buildGuardianFullAnalysis(
  db: IDatabase | null,
  tenantId: string | undefined,
  opts?: { windowDays?: number; useLlm?: boolean; historyDbAttached?: boolean },
): Promise<GuardianFullAnalysis | null> {
  if (!db) return null;
  ...
  if (summary.totalRequests > 0) {
    ...
    traffic.push('No proxy traffic recorded — connect MCP clients through Guardian.');
  }
  ...
  const base: Omit<GuardianFullAnalysis, 'markdown'> = {
  ...
  if (isFullAnalysisLlmEnabled(useLlm)) {
    ...
    if (llm) {
      const withLlm: Omit<GuardianFullAnalysis, 'markdown'> = {
        ...base,
        source: 'llm',
      ...
      const full = { ...withLlm, markdown: '' } as GuardianFullAnalysis;
      full.markdown = formatMarkdown(full);
      return full;
    }
  }

  const full: GuardianFullAnalysis = { ...base, markdown: '' };
  full.markdown = formatMarkdown(full);
  return full;
}
