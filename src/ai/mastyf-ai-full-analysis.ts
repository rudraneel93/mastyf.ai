...
function isFullAnalysisLlmEnabled(useLlm: boolean): boolean {
  ...
  if (process.env.MASTYF_AI_FULL_ANALYSIS_LLM === 'false') return false;
...
async function buildLlmNarrative(
  citations: FullAnalysisCitation[],
  sections: MastyfAiFullAnalysis['sections'],
  ...
  try {
    const result = await llm.generate(
      'You explain MCP Mastyf AI security and health to a non-technical operator. Use ONLY the cited facts. Write 6-10 short paragraphs in plain English covering: Protection status, Traffic, Security blocks, Learning/autopilot, and Recommended next steps. Every paragraph must include at least one citation ID in brackets like [exec:requests]. Do not invent numbers or server names.',
    ...
    Logger.debug(
      `[mastyf-ai-full-analysis] LLM skipped: ${err instanceof Error ? err.message : String(err)}`,
    );
    return null;
  }
}

function formatMarkdown(report: MastyfAiFullAnalysis): string {
  const lines: string[] = [
  ...
  const sectionTitles: Array<[keyof MastyfAiFullAnalysis['sections'], string]> = [
...
export async function buildMastyfAiFullAnalysis(
  db: IDatabase | null,
  ...
  const base: Omit<MastyfAiFullAnalysis, 'markdown'> = {
  ...
  if (isFullAnalysisLlmEnabled(useLlm)) {
    ...
    if (llm) {
      const withLlm: Omit<MastyfAiFullAnalysis, 'markdown'> = {
        ...base,
        source: 'llm',
      ...
      const full = { ...withLlm, markdown: '' } as MastyfAiFullAnalysis;
      full.markdown = formatMarkdown(full);
      return full;
    }
  }

  const full: MastyfAiFullAnalysis = { ...base, markdown: '' };
  full.markdown = formatMarkdown(full);
  return full;
}
