/**
 * `mastyf-ai start` — proxy + dashboard with sensible local defaults.
 */
...
export async function runStart(opts: StartOptions = {}): Promise<void> {
  ...
  if (!config) {
    ...
    console.error(chalk.dim('  1. mastyf-ai onboard --apply'));
    console.error(chalk.dim('  2. mastyf-ai start --config mastyf-ai-configs/filesystem.json\n'));
    process.exit(1);
  }

  const blockingMode =
    opts.blockingMode || process.env.MASTYF_AI_BLOCKING_MODE || 'block';
  const port = process.env.DASHBOARD_PORT || '4000';

  console.log(chalk.bold('\nMCP Mastyf AI — starting proxy + dashboard\n'));
  console.log(chalk.dim(`  Dashboard: http://localhost:${port}/`));
  console.log(chalk.dim(`  DB: ${process.env.MASTYF_AI_DB_PATH}`));
