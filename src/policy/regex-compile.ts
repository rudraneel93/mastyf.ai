...
const REGEX_EVAL_TIMEOUT_MS = parseInt(
  process.env.MASTYF_AI_REGEX_EVAL_TIMEOUT_MS || '50',
  10,
);
...
export function shouldRejectUnsafePolicyRegex(): boolean {
  if (process.env.MASTYF_AI_POLICY_REJECT_UNSAFE_REGEX === 'false') return false;
  if (process.env.MASTYF_AI_POLICY_REJECT_UNSAFE_REGEX === 'true') return true;
...
export function shouldUseRegexWorker(): boolean {
  if (process.env.MASTYF_AI_REGEX_USE_WORKER === 'false') return false;
  if (process.env.MASTYF_AI_REGEX_USE_WORKER === 'true') return true;
  return process.env.NODE_ENV === 'production';
}
