...
export function isStrictAllowlistRbac(): boolean {
  if (process.env['MASTYF_AI_STRICT_ALLOWLIST_RBAC'] === 'true') return true;
  if (process.env['MASTYF_AI_STRICT_ALLOWLIST_RBAC'] === 'false') return false;
...
export function validateAllowlistRbac(config: PolicyConfig): void {
  ...
  for (const rule of config.policy.rules) {
    ...
    if (!hasRbac) {
      ...
      if (strict) {
        throw new Error(
          `${msg}. Set MASTYF_AI_STRICT_ALLOWLIST_RBAC=false to allow, or add rbac to the rule.`,
        );
      }
