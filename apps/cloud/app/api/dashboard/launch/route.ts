...
function normalizeMastyfAiUrl(raw: string): string | null {
  try {
...
export async function POST(request: Request) {
  ...
  const body = (await request.json()) as { mastyfAiUrl?: string };
  const mastyfAiUrl = normalizeMastyfAiUrl(body.mastyfAiUrl ?? '');
  if (!mastyfAiUrl) {
    return NextResponse.json({ error: 'Valid mastyfAiUrl required (http/https)' }, { status: 400 });
  }
  ...
  const redirectUrl = `${mastyfAiUrl}/api/auth/cloud-exchange?token=${encodeURIComponent(plaintext)}`;
