...
export default async function ConnectMastyfAiPage() {
  ...
  const envBlock = `# Required on your self-hosted Mastyf AI host for cloud SSO
MASTYF_AI_MULTI_TENANT_ENABLED=true
MASTYF_AI_TENANT_ID=${ctx.org.slug}
MASTYF_AI_CONTROL_PLANE_URL=${appUrl()}
# Same value as AUTH_SECRET on mastyf-ai-cloud (Vercel → Environment Variables):
...
`;
  ...
  return (
    <main className="container">
      ...
      <h1>Connect self-hosted MastyfAi</h1>
      <p className="muted">
        Optional: open your local or remote Mastyf AI ops dashboard with a one-time SSO token. The
        cloud console (policy, API keys) does not need this.
      </p>

      <div className="card">
        <h2>Environment</h2>
        <p className="muted">Restart Mastyf AI after setting these variables.</p>
