...
/**
 * SOC_API_PORT / NEXT_PUBLIC_MASTYF_AI_API controls where the dashboard SPA
 * fetches its live data from.
 *
 * In development (npm run dev) the Next.js server rewrites /api/* → the
 * standalone MCP Mastyf AI SOC API server (default: http://localhost:4040).
 ...
 * from NEXT_PUBLIC_MASTYF_AI_API.  When serving the exported files the API is
 * expected to be on the same host/port (e.g. nginx proxies /api → backend).
 */

const SOC_API_PORT = process.env['SOC_API_PORT'] ?? process.env['MASTYF_AI_PORT'] ?? '4000';
...
const nextConfig: NextConfig = {
  ...
  ...(isDev
    ? {
        // Dev: proxy /api/* to the Mastyf AI SOC API backend
        async rewrites() {
          return [
            {
              source: '/api/:path*',
