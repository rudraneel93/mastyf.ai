...
import type { FleetInstance, FleetResponse } from '@/lib/mastyf-ai-api';
...
const SOURCE_LABELS: Record<string, string> = {
  postgres: 'PostgreSQL mastyf_ai_instances',
  'multi-sqlite': 'MASTYF_AI_FLEET_DB_PATHS',
...
export function FleetOverviewPanel({ fleet, meta }: Props) {
  ...
  return (
    <DashboardSection
      ...
      subtitle={`${sourceLabel}${meta?.region ? ` · region ${meta.region}` : ''} — same data as mastyf-ai fleet status`}
    >
      ...
      {fleet.length === 0 ? (
        <p className="muted">
          ...
          multi-replica registry, or <code>MASTYF_AI_FLEET_DB_PATHS</code> for comma-separated SQLite DBs.
        </p>
      ) : (
        <>
          ...
          <DataTablePro
            ...
            exportFilename="mastyf-ai-fleet.csv"
          />
        </>
      )}
    </DashboardSection>
