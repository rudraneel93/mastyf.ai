    ...
    <div className="auto-research-monitor">
      ...
      {!status?.llm.ok ? (
        <p className="status status-error banner-inline">
          ...
          <code>OLLAMA_BASE_URL</code>: {status?.llm.reason || 'Configure Ollama and set MASTYF_AI_LLM_ENABLED=true'}
        </p>
      ) : null}
      ...
      {lastJobZeroWrites ? (
        <p className="status status-warning banner-inline">
          ...
          {parsed?.skips.replayFailed ? (
            <> Corpus replay uses <code>MASTYF_AI_CORPUS_REPLAY_POLICY_PATH</code> (default: strict default-policy.yaml).</>
          ) : null}
        </p>
      ) : null}
      ...
      {!status?.features.autoResearchEnabled ? (
        <p className="hint banner-inline">
          Auto research disabled — set <code>MASTYF_AI_THREAT_RESEARCH_AUTO=true</code> and{' '}
