...
// and runs the MCP Mastyf AI server on the Render-assigned port
...
const server = http.createServer((req, res) => {
  ...
  res.end(`<html><body><h1>MCP MastyfAi</h1><p>${serverCard.description}</p><p>Tools: scan_security, audit_costs, check_health, full_report</p></body></html>`);
});

server.listen(PORT, '0.0.0.0', () => {
  console.log(`MCP Mastyf AI running on port ${PORT} (${process.env.NODE_ENV || 'production'})`);
