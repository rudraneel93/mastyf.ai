...
run(venvPython, ['adversarial-harness/python/run_comprehensive_eval.py'], {
  ...
  env: { PYTHONPATH: join(__dir, 'python'), MASTYF_AI_DISABLE_SEMANTIC: process.env.MASTYF_AI_DISABLE_SEMANTIC || '' },
});
...
run(venvPython, ['adversarial-harness/python/run_corpus.py'], {
  ...
  env: { PYTHONPATH: join(__dir, 'python'), MASTYF_AI_DISABLE_SEMANTIC: 'true' },
});
...
run('pnpm', ['exec', 'tsx', 'corpus/run-eval.ts'], {
  ...
  env: { MASTYF_AI_DISABLE_SEMANTIC: process.env.MASTYF_AI_DISABLE_SEMANTIC || '' },
});
