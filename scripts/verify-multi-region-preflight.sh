...
REGION="${MASTYF_AI_REGION:-us-east-1}"
...
if [ -n "${REDIS_URL:-}" ]; then
  node -e "
    import('ioredis').then(async ({ default: Redis }) => {
      ...
      if (rtt > 80 && [process.env.MASTYF_AI_MULTI_REGION_MODE].includes('active-active')) {
...
if [ -z "${MASTYF_AI_REGION:-}" ]; then
  echo "WARN: MASTYF_AI_REGION not set — fleet/charts may not group by region"
fi
