...
ROOT="$(pwd)"
export MASTYF_AI_DB_PATH="${MASTYF_AI_DB_PATH:-/tmp/mastyf-ai-verify-$$.db}"
rm -f "$MASTYF_AI_DB_PATH" "$MASTYF_AI_DB_PATH-wal" "$MASTYF_AI_DB_PATH-shm" 2>/dev/null || true
...
test -f "$ROOT/mastyf-ai-configs/fixture_echo.json"

echo "=== Policy dry-run ==="
node dist/cli.js proxy --policy policy-audit.yaml --dry-run || true

echo "=== Live proxy smoke (echo-test) ==="
CONFIG="$ROOT/mastyf-ai-configs/echo-test.json"
...
OUT=$( (printf '%s\n' "$INIT"; sleep 0.5; printf '%s\n' "$CALL") | \
  ...
  sh "$ROOT/scripts/mastyf-ai-proxy.sh" --config "$CONFIG" --policy policy-audit.yaml 2>/dev/null | tail -5)
