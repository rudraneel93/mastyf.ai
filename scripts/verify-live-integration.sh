...
ROOT="$(pwd)"
export MCP_GUARDIAN_DB_PATH="${MCP_GUARDIAN_DB_PATH:-/tmp/mcp-guardian-verify-$$.db}"
rm -f "$MCP_GUARDIAN_DB_PATH" "$MCP_GUARDIAN_DB_PATH-wal" "$MCP_GUARDIAN_DB_PATH-shm" 2>/dev/null || true
...
OUT=$( (printf '%s\n' "$INIT"; sleep 0.5; printf '%s\n' "$CALL") | \
  ...
  sh "$ROOT/scripts/guardian-proxy.sh" --config "$CONFIG" --policy policy-audit.yaml 2>/dev/null | tail -5)
