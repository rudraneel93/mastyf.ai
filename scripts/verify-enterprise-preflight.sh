...
echo "[enterprise-preflight] MCP Mastyf AI enterprise preflight"
...
if [ "${MASTYF_AI_STRICT_MODE:-}" != "true" ]; then
  warn "MASTYF_AI_STRICT_MODE is not true — multi-replica may misbehave without Redis"
fi
...
if [ -n "${DATABASE_URL:-}" ]; then
  case "$DATABASE_URL" in
    *pgbouncer*|*:6432*)
      ...
      ;;
    *)
      if [ "${MASTYF_AI_REQUIRE_PGBOUNCER:-}" = "true" ]; then
        fail "MASTYF_AI_REQUIRE_PGBOUNCER=true but DATABASE_URL is not PgBouncer"
      else
...
if [ -f deploy/helm/mastyf-ai/values-enterprise.yaml ]; then
  ...
  fail "missing deploy/helm/mastyf-ai/values-enterprise.yaml"
fi
