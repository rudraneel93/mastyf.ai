 ...
 * Reads persisted records from PostgreSQL (when configured) and ~/.mastyf-ai JSONL fallback.
 */
...
async function main(): Promise<void> {
  ...
  const currentMin = parseFloat(process.env.MASTYF_AI_SEMANTIC_MIN_CONFIDENCE || '0.6');
  const currentLocal = parseFloat(process.env.MASTYF_AI_LOCAL_SEMANTIC_THRESHOLD || '0.55');
  ...
  const emptyReason =
    records.length === 0
      ? llmConfigured
        ...
        : 'Semantic LLM not configured (set OPENAI_API_KEY or ANTHROPIC_API_KEY and MASTYF_AI_LLM_ENABLED). Async semantic audits will not produce calibration data.'
      : undefined;
  ...
  const report = {
    ...
    thresholds: {
      current: {
        MASTYF_AI_SEMANTIC_MIN_CONFIDENCE: currentMin,
        MASTYF_AI_LOCAL_SEMANTIC_THRESHOLD: currentLocal,
      },
      recommended: {
        MASTYF_AI_SEMANTIC_MIN_CONFIDENCE: Math.round(recommendedMin * 1000) / 1000,
        MASTYF_AI_LOCAL_SEMANTIC_THRESHOLD: Math.round(recommendedLocal * 1000) / 1000,
      },
      note: 'Apply manually or via tenant config; auto-apply requires quorum (MASTYF_AI_AI_AUTO_APPLY stays false in prod).',
    },
    profile:
  ...
  console.log(
    `Recommended MASTYF_AI_SEMANTIC_MIN_CONFIDENCE: ${report.thresholds.recommended.MASTYF_AI_SEMANTIC_MIN_CONFIDENCE}`,
  );
  console.log(
    `Recommended MASTYF_AI_LOCAL_SEMANTIC_THRESHOLD: ${report.thresholds.recommended.MASTYF_AI_LOCAL_SEMANTIC_THRESHOLD}`,
  );
