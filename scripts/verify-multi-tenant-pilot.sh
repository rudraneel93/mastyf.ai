...
if [ "${MASTYF_AI_MULTI_TENANT_ENABLED:-}" != "true" ]; then
  warn "MASTYF_AI_MULTI_TENANT_ENABLED is not true — set for production pilot"
fi
...
if curl -sf "${BASE_URL}/api/aggregate/audit" -H 'X-Mastyf-Ai-Tenant: tenant-a' >/tmp/pilot-a.json 2>/dev/null; then
