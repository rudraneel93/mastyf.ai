...
function setupEnv(dir: string, instant: boolean): void {
  ...
  process.env.MASTYF_AI_AI_INSTANT_LEARNING = instant ? 'true' : 'false';
  process.env.MASTYF_AI_AI_ATTACK_MIN_BLOCKS = '3';
  process.env.MASTYF_AI_AI_INSTANT_WINDOW_MS = String(REPEAT_WINDOW_MS);
  process.env.MASTYF_AI_AI_BLOCK_DEBOUNCE_MS = String(DEBOUNCE_MS);
...
export function runInstantScenario(events: SimBlockEvent[], topClusters = 15): ScenarioMetrics {
  const dir = mkdtempSync(join(tmpdir(), 'mastyf-ai-eval-instant-'));
  setupEnv(dir, true);
...
export function runBatchScenario(events: SimBlockEvent[], topClusters = 15): ScenarioMetrics {
  const dir = mkdtempSync(join(tmpdir(), 'mastyf-ai-eval-batch-'));
  setupEnv(dir, false);
