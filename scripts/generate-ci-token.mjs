#!/usr/bin/env node
/**
 * Generate a MASTYF_AI_CI_TOKEN (Ed25519-signed JWT) for CI pipelines.
 *
 * Usage:
 ...
 *
 * Requires: MASTYF_AI_CI_PRIVATE_KEY env var (Ed25519 JWK) or
 * a key file at the path in MASTYF_AI_CI_KEY_FILE.
 *
 * The private key is NEVER embedded in the source or dist.
 * Only the public key lives in src/license/ci-token.ts.
 *
 * Output: base64url-encoded JWT to stdout. Set as MASTYF_AI_CI_TOKEN.
 */
...
async function getPrivateKey() {
  // 1. Try env var
  if (process.env['MASTYF_AI_CI_PRIVATE_KEY']) {
    return parseJwk(process.env['MASTYF_AI_CI_PRIVATE_KEY']);
  }

  // 2. Try key file
  const keyFile = process.env['MASTYF_AI_CI_KEY_FILE'];
  if (keyFile && existsSync(keyFile)) {
    return parseJwk(readFileSync(keyFile, 'utf-8'));
  }

  console.error('[ci-token] No private key found. Set MASTYF_AI_CI_PRIVATE_KEY or MASTYF_AI_CI_KEY_FILE.');
...
async function main() {
  ...
  console.error(`  Usage: export MASTYF_AI_CI_TOKEN="${token}"`);
  console.error(`  Or add to CI secrets as MASTYF_AI_CI_TOKEN`);
}
