...
const dbPath = process.env.MASTYF_AI_DB_PATH || path.join(os.homedir(), '.mastyf-ai', 'history.db');
...
if (!status.rotationEnabled) {
  console.log('[rotate-field-encryption] rotation disabled; set MASTYF_AI_DB_ENCRYPTION_ROTATION_ENABLED=true');
  process.exit(0);
}
