...
docker build --target builder -t mastyf-ai-prebuild-check:local -f Dockerfile .

echo "==> Running better-sqlite3 smoke inside builder image..."
docker run --rm mastyf-ai-prebuild-check:local \
...
docker build -t mastyf-ai-nonroot-check:local -f Dockerfile .
uid="$(docker run --rm --entrypoint id mastyf-ai-nonroot-check:local)"
echo "$uid"
